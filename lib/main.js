...
export default {
    ...
    async onNameEditorConfirm(msg) {
        ...
        switch (msg.type) {
            case 'newFile':
                newPath = _.getNewFilePath(this.currentPath, msg.value);
                return this.onNewFileConfirm(newPath);
            case 'newFolder':
                newPath = _.getNewFolderPath(this.currentPath, msg.value);
    ...
    async onNewFileConfirm(pathname) {
        console.info('new file:', pathname);
        try {
            await createFile(pathname);
            this.updateListeningList();
            this.nameEditor.hide();
        } catch (e) {
            notify.error('Creating new file failed.');
        }
    },
    onNewFolderConfirm(pathname) {
        console.info('new folder:', pathname);
    },

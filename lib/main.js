...
export default {
    ...
    initMenu() {
        ...
        this.menu.on('newFile', (msg) => {
            this.onNewFile(msg);
        });
        this.menu.on('newFolder', (msg) => {
            this.onNewFolder(msg);
        });
        ...
        this.menu.on('findAndReplace', (msg) => {
            this.onFindAndReplace();
        });
        this.menu.on('searchInDirectory', async (msg) => {
            console.info('find in directory');
            await this.toggleFindInProject();
        });
    },
    triggerInSingleBtnMode(msg) {
        switch (config.singleButtonType) {
            case 'newFile':
                return this.onNewFile(msg);
            case 'newFolder':
                return this.onNewFolder(msg);
            case 'copyFullPath':
            ...
            case 'findAndReplace':
                return this.onFindAndReplace(msg);
            case 'searchInDirectory':
                return this.toggleFindInProject();
            default:
                console.error('Invalid type', config.singleButtonType);
        }
    },
    onFindAndReplace(msg) {
        // TODO activate clicked file
        console.info('find and replace', msg);
        console.info(this.currentPath);
        atom.workspace.open(this.currentPath);
        atom.commands.dispatch(this.currentNode, 'find-and-replace:toggle');
    },
    async toggleFindInProject() {
        if (!this.findAndReplaceInited) { // to get the package ref, we have to activate it first
            atom.commands.dispatch(this.currentNode.parentNode, 'project-find:show');
        } else {
            atom.commands.dispatch(this.currentNode.parentNode, 'project-find:toggle');
        ...
        try {
            ...
            const rootPath = this.currentNode.dataset.root;
            const closestDir = _.getClosestDir(this.currentPath);
            const relativePath = path.relative(rootPath, closestDir);
            pathsEditor.setText(relativePath);
    ...
    updateListeningList(delay) {
        ...
        setTimeout(() => {
            this.treeView.roots.forEach((root) => {
                const pathNode = root.querySelector('.icon-repo');

...
export default Object.assign({}, {
        ...
        Object.keys(this.config).forEach((key) => {
            atom.config.observe(`${packageName}.${key}`, (value) => {
                ...
                if (key.includes('-alias')) {
                    ...
                    return this.updateMenuBtnName(btnId, value)
                }
    ...
    updateMenu(btnId, enable) {
        if (enable) {
            return this.menu.addButton(btnId, {
                name: configCache[`${btnId}-alias`],
            })
        }
        this.menu.removeButton(btnId)
    },
    updateMenuBtnName(btnId, name) {
        this.menu.updateBtnName(btnId, name)
    },
    resizeMenu() {
    ...
    onRename(msg) {
        this.nameEditor.update({
            position: msg.position,
            entryName: path.basename(this.currentPath),
            type: 'rename',
        })
        this.nameEditor.show()
    },

...
export default {
    ...
    async ignore(pathname, ignoreType) {
        try {
            const ignoreText = await this.generateIgnoreText(pathname, ignoreType)
    ...
    async generateIgnoreText(pathname, ignoreType) {
        ...
        switch (ignoreType) {
            ...
            case ignoreTypes.ignoreSpecificFolder.value:
                return path.sep + path.relative(ignoreFileFolder, pathname) + path.sep + EOL
